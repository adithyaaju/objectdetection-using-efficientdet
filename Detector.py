...
class Detector:
    ...
    def predictImage(self, imagePath, threshold = 0.5):
        ...
        image1 = cv2.resize(bboxImage, (1080, 720))
    ...
    def predictVideo(self, videoPath, threshold = 0.5):
        ...
        while success:
            ...
            inf_time1 = (end_time - start_time)
            fps = 1/inf_time1
            start_time = end_time
            fps_text = "FPS: {:.2f}".format(fps)
            inf_text = "Inference time: {:.2f}".format(inf_time)


            bboxImage = self.createBoundingBox(image, threshold)
            cv2.putText(bboxImage, fps_text, (20, 70), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 255, 0), 2)
            cv2.putText(bboxImage, inf_text, (20, 180), cv2.FONT_HERSHEY_DUPLEX, 2, (255, 0, 0), 2)

            output_video.write(bboxImage)
            bboxImage1 = cv2.resize(bboxImage, (7680, 4320))
            cv2.imshow("Result", bboxImage1)
